...
const SEMI_OBJ_PCT = 87

function formatSemiObjPace(vma, { suffix = true } = {}) {
  const p = formatPace(vma, SEMI_OBJ_PCT)
...
function renderHtml({ vma, seances }) {
  ...
  const SEMAINES = require(`../ebook-data/semi-12sem/semaines-${seances}-${tier}.js`)
  const ZONES = buildZones(vma)
  const objPace = formatSemiObjPace(vma, { suffix: false })
...
<div class="page">
  ...
  <div class="som-flat">
    ...
    ${[
      ...
      ['Semaines 9-10 · Spécificité semi','13-14'],
    ...
    ${[['Stratégie de course','17'],['Nutrition avant et après','18'],['Coaching personnalisé','19']].map(([l,p])=>
      `<div class="som-item"><span class="som-label">${l}</span><span class="som-dots"></span><span class="som-page">p.${p}</span></div>`
    ).join('')}
  </div>
...
<div class="page">
  ...
  <p class="page-intro">Les règles qui font la différence entre finir un semi-marathon et le réussir.</p>
  <div class="principes-wrap">
    ${[
      ...
      ['03','Le retour au calme, 10 min, toujours','10 minutes de jogging léger après chaque séance intense. Accélère l\'élimination des déchets métaboliques et prépare le corps pour la séance suivante. Sur 12 semaines, l\'effet cumulé est significatif.'],
...
<div class="page">
  ...
  <div class="nut-cards-wrap">
    ...
    <div class="nut-card" style="border-left:3px solid rgba(245,158,11,.7);background:rgba(245,158,11,.07)">
      ...
      <div class="nut-items">
        ${[['Que manger','Pain blanc + confiture ou miel, banane, flocons d\'avoine.'],['Gel ou barre','Si ta course dure plus d\'1h30, prends 1 gel testés à l\'entraînement au km 12-14.'],['Hydratation','500 ml d\'eau jusqu\'au départ. Arrête 30 min avant.'],['Timing','Dernier repas obligatoirement 2h30-3h avant le départ.']].map(([k,v])=>`
        <div class="nut-item">
