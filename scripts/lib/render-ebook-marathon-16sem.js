...
const { formatPace, formatPaceRange, formatSpeed } = require('./paces')

const MARATHON_OBJ_PCT = 80

function formatMarathonObjPace(vma, { suffix = true } = {}) {
  const p = formatPace(vma, MARATHON_OBJ_PCT)
  return suffix ? `${p}/km` : p
}
...
.pnum { position: absolute; bottom: 11mm; right: 13mm; z-index: 10; }
.page-title { flex-shrink: 0; }
.page-intro { font-size: 9.5pt; color: rgba(255,255,255,.72); line-height: 1.6; text-align: center; flex-shrink: 0; }
.section-label { font-size: 7pt; font-weight: 700; text-transform: uppercase; letter-spacing: .1em; color: rgba(255,255,255,.38); margin-bottom: 6px; }
.cover-page {
...
.cover-wm {
  position: absolute; font-weight: 800; line-height: 1; letter-spacing: -.03em;
...
.cover-title { font-size: 52pt; font-weight: 800; line-height: .9; letter-spacing: -.03em; }
...
.principe { flex: 1; display: flex; gap: 14px; align-items: center; padding: 6px 12px; overflow: hidden; }
...
.strat-km { width: 60px; flex-shrink: 0; text-align: center; }
.strat-km-l { font-size: 10pt; font-weight: 800; line-height: 1.1; display: inline-block; color: #C084FC; }
...
.week-hero { flex-shrink: 0; padding-bottom: 8px; text-align: center; }
.week-num { margin-bottom: 2px; }
.week-phase { margin-bottom: 2px; }
.week-charge { font-size: 9pt; color: rgba(255,255,255,.45); }
.objectif { flex-shrink: 0; padding: 7px 0 7px 11px; border-left: 2px solid rgba(139,47,201,.55); }
...
.session { flex: 1; min-height: 0; display: flex; flex-direction: column; border-radius: 12px; overflow: hidden; }
.s-head { flex-shrink: 0; position: relative; height: 40px; display: flex; align-items: center; padding: 0 13px; }
...
.nut-card { flex: 1; display: flex; flex-direction: column; min-height: 0; padding: 11px 14px; border-radius: 12px; overflow: hidden; }
...
.fin-feat { background: rgba(255,255,255,.04); border: 1px solid rgba(255,255,255,.07); border-radius: 12px; padding: 14px; display: flex; flex-direction: column; }
...
.fin-icon { width: 38px; height: 38px; border-radius: 10px; background: linear-gradient(135deg,#8B2FC9,#E8237A); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
function fillText(text, pctQueue, vma) {
  ...
  out = out.replace(/{{OBJ}}/g, () => HL(`${formatMarathonObjPace(vma, { suffix: false })}/km`))
  out = applyHL(out)
  return out
}
...
function weekPage(sem, vma) {
  ...
  <div class="page ${pageDense}">
    ...
    <div class="week-hero">
      <div class="week-num">${gradText(`Semaine ${sem.num} sur 16`, { sizePt: 11, weight: 700, letterSpacing: '0.07em', uppercase: true })}</div>
...
const ZONE_DEFS = [
  { pct:65,               name:'Endurance Fondamentale',     desc:'Allure conversation · footing EF · 70-80% du volume total', c:'#06B6D4', rgb:'6,182,212' },
  { pct:75,               name:'Endurance Active',            desc:'Confortable mais actif · soutenable sur 2h+', c:'#10B981', rgb:'16,185,129' },
  { pct:MARATHON_OBJ_PCT, name:'Allure objectif marathon',    desc:'Ta cible le jour de la course · à mémoriser absolument', c:'#FBBF24', rgb:'251,191,36', obj:true },
...
function vmaTier(vma) {
  if (vma <= 13.5) return 'd'
  if (vma <= 18.5) return 'i'
  return 'a'
}

function renderHtml({ vma, seances }) {
  const tier = vmaTier(vma)
  const SEMAINES = require(`../ebook-data/marathon-16sem/semaines-${seances}-${tier}.js`)
  const ZONES = buildZones(vma)
  const objPace = formatMarathonObjPace(vma, { suffix: false })
...
<div class="cover-page">
  <div class="cover-wm" style="font-size:130pt">MARATHON</div>
  ...
  <div class="cover-inner">
    ...
    <div class="cover-title"><span style="display:inline-block;background:linear-gradient(135deg,#8B2FC9,#E8237A);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text">MARATHON</span></div>
    <div class="cover-subtitle">16 semaines · ${seances} séances/semaine · VMA ${vma} km/h</div>
...
<div class="page">
  ...
  <div class="som-flat">
    ...
    ${[['Tes allures personnalisées','3'],['Les principes du plan','4']].map(([l,p])=>
    ...
    ${[
      ['Semaines 1-4 · Fondamentaux','5-8'],
      ['Semaines 5-8 · Développement','9-12'],
      ['Semaines 9-12 · Spécificité marathon','13-16'],
      ['Semaines 13-14 · Pic de charge','17-18'],
      ['Semaine 15 · Consolidation','19'],
      ['Semaine 16 · Semaine de course','20'],
    ].map(([l,p])=>
      `<div class="som-item"><span class="som-label">${l}</span><span class="som-dots"></span><span class="som-page">p.${p}</span></div>`
    ).join('')}
    <div class="som-glbl">Course &amp; après</div>
    ${[['Stratégie de course','21'],['Nutrition avant et pendant','22'],['Coaching personnalisé','23']].map(([l,p])=>
      `<div class="som-item"><span class="som-label">${l}</span><span class="som-dots"></span><span class="som-page">p.${p}</span></div>`
    ).join('')}
  </div>
...
<div class="page">
  ...
  <div style="flex-shrink:0;text-align:center;font-size:7.5pt;font-weight:700;color:#FBBF24;text-transform:uppercase;letter-spacing:.09em">
    Ton allure objectif marathon : ${objPace}/km
  </div>
...
<div class="page">
  ...
  <p class="page-intro">Le marathon est la distance la plus exigeante en endurance. Ces principes font la différence entre courir 42 km et les réussir.</p>
  <div class="principes-wrap">
    ${[
      ['01','L\'endurance fondamentale, le socle du marathon','70 à 80% de ton volume total s\'effectue en Endurance Fondamentale. C\'est la base sur laquelle tout repose. Le marathon est avant tout une épreuve aérobie, et cette base se construit patiemment sur 16 semaines.'],
      ['02','La sortie longue, sacro-sainte','La sortie longue du dimanche est la séance la plus importante de la semaine. Elle développe les adaptations métaboliques spécifiques au marathon : utilisation des graisses, résistance glycogénique, habitude du long effort.'],
      ['03','La nutrition à l\'entraînement dès 90 minutes','Dès que tu dépasses 90 minutes de course, consomme 1 gel énergétique toutes les 30-35 minutes. Teste tes gels à l\'entraînement, jamais pour la première fois en course. L\'intestin s\'entraîne comme les jambes.'],
      ['04','La récupération, partie intégrante du plan','Le marathon se prépare en dehors des séances autant qu\'en courant. Sommeil 8h minimum les nuits précédant les séances clés, hydratation quotidienne, alimentation riche en glucides complexes. Sans récupération, pas d\'adaptation.'],
      ['05','Le renforcement musculaire, obligatoire','1 séance de 20 min par semaine en complément des '+seances+' séances de ce plan : gainage profond, fentes, squats, montées de mollets. Le marathon sollicite chaque muscle du corps sur 3 à 5 heures d\'effort.'],
    ].map(([n,t,d])=>`
    <div class="principe">
...
<div class="page">
  ...
  <div style="flex-shrink:0;padding:8px 0 8px 11px;border-left:2px solid rgba(139,47,201,.4);font-size:9.5pt;color:rgba(255,255,255,.75);line-height:1.5">
    <strong style="color:#C084FC">Gel strategy :</strong> 1 gel toutes les 7-8km à partir du km 14-16. Teste cette stratégie sur tes longues sorties. L'intestin doit être habitué le jour J.
  </div>
  <div class="pnum">${gradText('21', { sizePt: 14, weight: 800, width: 40, align: 'end' })}</div>
</div>
...
<div class="page">
  ...
  <p class="page-intro">Le marathon dure entre 3h et 5h. La gestion de l'énergie et de l'hydratation est aussi déterminante que l'entraînement. Les 72h avant le départ sont cruciales.</p>
  <div class="nut-cards-wrap">
    <div class="nut-card" style="border-left:3px solid rgba(168,85,247,.7);background:rgba(168,85,247,.07)">
      <div class="nut-head">
        ...
        <div>
          <div class="nut-title">Les 72h avant la course</div>
          <div class="nut-timing">J-3 à J-1 · Charge glucidique progressive</div>
        </div>
      </div>
      <div class="nut-items">
        ${[['Charge glucidique','Augmente les glucides progressivement sur 3 jours : riz, pâtes, pain, pommes de terre. Objectif : saturer les stocks de glycogène.'],['Aliments à éviter','Légumineuses, crudités, fibres, alcool. Risque digestif en course. Aucun nouvel aliment dans les 72h.'],['Veille du marathon','Dîner léger et facile à digérer. Pâtes sauce tomate ou riz vapeur. Rien de gras, rien de lourd.'],['Hydratation','1,5 à 2L d\'eau par jour. Ajouter une boisson isotonique la veille.']].map(([k,v])=>`
        <div class="nut-item">
    ...
    <div class="nut-card" style="border-left:3px solid rgba(245,158,11,.7);background:rgba(245,158,11,.07)">
      <div class="nut-head">
        ...
        <div>
          ...
          <div class="nut-timing">Jour J · Dernier repas 3h avant</div>
        </div>
      </div>
      <div class="nut-items">
        ${[['Petit déjeuner','Pain blanc ou toast + confiture/miel + banane. Pas d\'œufs, pas de gras, pas de fibres.'],['Gels pendant','1 gel toutes les 7-8km à partir du km 14. Préférence gels caféinés à partir du km 25-28.'],['Hydratation matinale','500-750ml d\'eau jusqu\'au départ. Arrête 20-30 min avant le coup de feu.'],['Timing','Dernier repas obligatoirement 3h avant. Si départ à 9h, petit déj à 6h maximum.']].map(([k,v])=>`
        <div class="nut-item">
    ...
    <div class="nut-card" style="border-left:3px solid rgba(34,197,94,.7);background:rgba(34,197,94,.07)">
      ...
      <div class="nut-items">
        ${[['30 min','Boisson de récupération ou lait chocolaté + banane. Protéines + glucides rapides.'],['2 heures','Repas complet : riz, pâtes, pommes de terre + protéines (poulet, omelette, poisson).'],['Alcool','Évite strictement les 48h suivantes. L\'alcool bloque la synthèse protéique et la réhydratation.'],['Jours suivants','Continue à manger des glucides et protéines. La récupération d\'un marathon prend 3 à 4 semaines.']].map(([k,v])=>`
        <div class="nut-item">
  ...
  <div class="pnum">${gradText('22', { sizePt: 14, weight: 800, width: 40, align: 'end' })}</div>
</div>
...
<div class="page page-fin">
  ...
  <div class="pnum">${gradText('23', { sizePt: 14, weight: 800, width: 40, align: 'end' })}</div>
</div>
