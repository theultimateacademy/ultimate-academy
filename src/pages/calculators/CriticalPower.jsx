...
const FAQ = [
  ...
  { q:'Comment réaliser les tests de puissance ?', a:"Les tests de 5 minutes et 20 minutes doivent être réalisés à effort maximal soutenable, pas à fond dès le départ. Pour le 5 minutes : part à un effort très intense mais que tu peux tenir jusqu'au bout. Pour le 20 minutes : commence légèrement en dessous de ce que tu penses pouvoir tenir, puis gère ton effort. Chaque test doit être séparé par au moins 24h de récupération." },
  { q:'Le rapport W/kg est-il vraiment important ?', a:"En cyclisme, le rapport W/kg est le roi de la performance en montagne. Deux cyclistes avec la même CP mais des poids différents n'auront pas les mêmes performances en côte : celui avec le meilleur rapport W/kg sera avantageux. Sur le plat, la puissance absolue (watts) prime. En contre-la-montre sur parcours vallonné, les deux comptent." },
          ...
          <div style={{ display:'flex', gap:'3rem', alignItems:'center', flexWrap:'wrap' }}>
            <div style={{ flex:'1 1 380px', borderRadius:20, overflow:'hidden', maxHeight:440 }}>
              <svg viewBox="0 0 440 300" style={{width:'100%',height:'100%',display:'block',minHeight:280}}>
                <defs>
                  <linearGradient id="cpg1" x1="0" y1="1" x2="1" y2="0">
                    <stop offset="0%" stopColor="#8B2FC9"/><stop offset="100%" stopColor="#E8237A"/>
                  </linearGradient>
                </defs>
                <rect width="440" height="300" fill="#EDE8F7"/>
                <text x="220" y="26" textAnchor="middle" fontSize="12" fill="#1a1230" fontWeight="800" fontFamily="system-ui,sans-serif">Modèle Monod & Scherrer (1965)</text>
                <text x="220" y="42" textAnchor="middle" fontSize="9.5" fill="rgba(26,18,48,.45)" fontFamily="system-ui,sans-serif">P(t) = CP + W' / t — Relation puissance–durée</text>
                {/* Axes */}
                <line x1="55" y1="55" x2="55" y2="225" stroke="rgba(26,18,48,.15)" strokeWidth="1.5"/>
                <line x1="55" y1="225" x2="410" y2="225" stroke="rgba(26,18,48,.15)" strokeWidth="1.5"/>
                <text x="28" y="60" textAnchor="middle" fontSize="9" fill="rgba(26,18,48,.4)" fontFamily="system-ui,sans-serif">Pwr</text>
                <text x="240" y="246" textAnchor="middle" fontSize="9" fill="rgba(26,18,48,.4)" fontFamily="system-ui,sans-serif">Durée →</text>
                {/* CP asymptote */}
                <line x1="55" y1="168" x2="408" y2="168" stroke="#8B2FC9" strokeWidth="1.5" strokeDasharray="6,3"/>
                <text x="415" y="171" fontSize="9.5" fill="#8B2FC9" fontWeight="700" fontFamily="system-ui,sans-serif">CP</text>
                {/* Hyperbolic curve P(t) = CP + W'/t */}
                <path d="M65,72 Q80,85 100,105 Q130,130 165,148 Q200,158 240,163 Q290,167 350,168"
                  fill="none" stroke="url(#cpg1)" strokeWidth="3" strokeLinecap="round"/>
                {/* W' zone fill */}
                <path d="M65,72 L65,168 Q100,168 130,168 Q130,130 100,105 Q80,85 65,72 Z"
                  fill="rgba(232,35,122,.08)"/>
                <text x="85" y="127" textAnchor="middle" fontSize="11" fill="#E8237A" fontWeight="700" fontFamily="system-ui,sans-serif">W'</text>
                <text x="85" y="141" textAnchor="middle" fontSize="8.5" fill="rgba(232,35,122,.7)" fontFamily="system-ui,sans-serif">kJ réserve</text>
                {/* Test points */}
                <circle cx="100" cy="202" r="0" fill="none"/>
                <circle cx="128" cy="148" r="5.5" fill="#E8237A" stroke="#fff" strokeWidth="1.5"/>
                <text x="135" y="143" fontSize="9" fill="#E8237A" fontWeight="700" fontFamily="system-ui,sans-serif">5 min</text>
                <circle cx="200" cy="158" r="5.5" fill="#8B2FC9" stroke="#fff" strokeWidth="1.5"/>
                <text x="207" y="153" fontSize="9" fill="#8B2FC9" fontWeight="700" fontFamily="system-ui,sans-serif">20 min</text>
                {/* FTP note */}
                <line x1="55" y1="181" x2="408" y2="181" stroke="rgba(26,18,48,.12)" strokeWidth="1" strokeDasharray="4,4"/>
                <text x="415" y="184" fontSize="8.5" fill="rgba(26,18,48,.4)" fontFamily="system-ui,sans-serif">FTP</text>
                {/* Zones legend */}
                {[['I1-I3','#34D399',195],['I4','#FBBF24',210],['I5-I7','#EF4444',225]].map(([l,col,y])=>(
                  <g key={l}>
                    <rect x="260" y={y-9} width="12" height="9" rx="2" fill={col} opacity=".7"/>
                    <text x="278" y={y} fontSize="8.5" fill="rgba(26,18,48,.55)" fontFamily="system-ui,sans-serif">{l}</text>
                  </g>
                ))}
                <text x="220" y="272" textAnchor="middle" fontSize="9.5" fill="rgba(26,18,48,.4)" fontFamily="system-ui,sans-serif" fontStyle="italic">Monod H. & Scherrer J., Ergonomics, 1965</text>
              </svg>
            </div>
      ...
      <section style={{ background:'#0C0A18', padding:'5rem 1.5rem' }}>
        <Inner max={900}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(250px,1fr))', gap:'1.25rem', marginBottom:'2rem' }}>
            {[
              { t:'Test 5 minutes', icon:'⚡', d:"Après un échauffement de 15-20 min incluant quelques efforts courts, réalise un effort à intensité maximale soutenable pendant exactement 5 minutes. Récupère 48h avant le test suivant." },
              { t:'Test 20 minutes', icon:'🔥', d:"Le classique du cyclisme. Après un échauffement complet, maintiens la puissance la plus élevée que tu puisses soutenir pendant 20 minutes sans baisser. FTP = 95% de ce résultat." },
              { t:'Test 12 minutes (optionnel)', icon:'📊', d:"Ajoute ce test pour améliorer la précision du modèle. Même protocole : effort maximal soutenable sur 12 minutes. Avec 3 points, la régression donne une CP plus fiable." },
              { t:'Conditions du test', icon:'🎯', d:"Toujours en forme, jamais en fatigue accumulée. Sur home trainer pour plus de répétabilité. Nutrition normale la veille. Ne jamais tester deux durées le même jour." },
            ].map(s=>(
