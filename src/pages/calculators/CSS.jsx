...
const inputSt = { background:'rgba(255,255,255,.07)', border:'1px solid rgba(139,47,201,.25)', borderRadius:10, padding:'.55rem .75rem', color:C.dark, fontSize:'.9rem', outline:'none', width:80 }
      ...
      <section ref={calcRef} id="outil" style={{ background:'#000', padding:'5rem 1.5rem' }}>
        <Inner max={760}>
          <div style={{ textAlign:'center', marginBottom:'2.5rem' }}>
            ...
            <p style={{ color:'rgba(255,255,255,.5)', fontSize:'.9rem' }}>Deux tests suffisent : un 200m et un 400m à allure maximale, avec 10-15 min de récupération entre les deux</p>
          </div>
          ...
          <div style={{ background:'rgba(255,255,255,.05)', borderRadius:20, padding:'2rem', boxShadow:'0 8px 40px rgba(139,47,201,.1)', border:'1px solid rgba(139,47,201,.12)' }}>
            ...
            {valid ? (
              <>
                ...
                <div style={{ background:`linear-gradient(135deg,rgba(139,47,201,.1),rgba(232,35,122,.07))`, borderRadius:16, padding:'1.5rem', border:'1px solid rgba(139,47,201,.15)', marginBottom:'1.75rem', display:'flex', alignItems:'center', gap:'1.5rem', flexWrap:'wrap' }}>
                  <div>
                    ...
                    <p style={{ fontSize:'2.5rem', fontWeight:900, color:C.dark, margin:0, fontVariantNumeric:'tabular-nums' }}>{secToMMSS(cssPace)}<span style={{ fontSize:'1rem', fontWeight:400, color:'rgba(255,255,255,.45)', marginLeft:'.3rem' }}>/100m</span></p>
                  </div>
                  <div style={{ width:1, height:56, background:'rgba(139,47,201,.15)', flexShrink:0 }}/>
                  <div>
                    <p style={{ fontSize:'.75rem', color:'rgba(255,255,255,.45)', textTransform:'uppercase', letterSpacing:'.08em', marginBottom:'.3rem' }}>1 500m estimé à CSS</p>
                    ...
                    <tbody>
                      {CSS_ZONES.map(z=>{
                        ...
                        return <tr key={z.n} style={{ borderTop:'1px solid rgba(139,47,201,.07)' }}>
                          ...
                          <td style={{ padding:'.6rem .75rem', color:'rgba(255,255,255,.45)', fontVariantNumeric:'tabular-nums' }}>
                            {z.n === 1 ? `< ${pctHi}%` : z.n === 5 ? `> ${pctLo}%` : `${pctLo} – ${pctHi}%`}
                          </td>
                          <td style={{ padding:'.6rem .75rem', color:'rgba(255,255,255,.5)', fontSize:'.82rem' }}>{z.desc}</td>
                        </tr>
                      })}
                    </tbody>
                  </table>
                ...
                <p style={{ color:'rgba(255,255,255,.45)', fontSize:'.82rem', marginBottom:'.75rem' }}>Temps estimés à ton allure CSS exacte. Pour les distances triathlon, la colonne "% CSS en course" indique l'intensité réaliste en compétition.</p>
                    ...
                    <tbody>
                      {SWIM_DISTS.map((d,i)=>{
                        ...
                        return <tr key={d.m} style={{ borderTop:'1px solid rgba(139,47,201,.07)', background:i%2===0?'transparent':'rgba(139,47,201,.02)' }}>
                          ...
                          <td style={{ padding:'.6rem .85rem', color:'rgba(255,255,255,.4)', fontSize:'.82rem' }}>{d.note || ''}</td>
                ...
                <p style={{ fontSize:'.75rem', color:'rgba(255,255,255,.35)', marginTop:'.6rem' }}>Source allures triathlon : Laursen P.B. et al., Int J Sports Physiol Perform, 2006</p>
              </>
            ) : (
              <div style={{ padding:'2.5rem', textAlign:'center', borderRadius:12, background:'rgba(139,47,201,.04)', border:'1px dashed rgba(139,47,201,.15)' }}>
                <p style={{ color:'rgba(255,255,255,.35)', margin:0 }}>
            ...
            <div style={{ flex:'1 1 320px' }}>
              <div style={{ background:'rgba(255,255,255,.04)', borderRadius:20, padding:'2rem', border:'1px solid rgba(139,47,201,.12)' }}>
                ...
                <div style={{ background:'rgba(255,255,255,.05)', borderRadius:14, padding:'1.5rem', textAlign:'center', marginBottom:'1.25rem', border:'1px solid rgba(139,47,201,.1)' }}>
                  ...
                  <p style={{ color:'rgba(255,255,255,.4)', fontSize:'.78rem', margin:'.4rem 0 0' }}>résultat en secondes par 100m</p>
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'.75rem', marginBottom:'1rem' }}>
                  <div style={{ background:'rgba(232,35,122,.06)', borderRadius:10, padding:'.9rem', border:'1px solid rgba(232,35,122,.12)' }}>
                    ...
                    <p style={{ color:'rgba(255,255,255,.6)', fontSize:'.8rem', lineHeight:1.55, margin:0 }}>Ton temps sur 400m à effort maximal soutenable</p>
                  </div>
                  <div style={{ background:'rgba(139,47,201,.06)', borderRadius:10, padding:'.9rem', border:'1px solid rgba(139,47,201,.12)' }}>
                    ...
                    <p style={{ color:'rgba(255,255,255,.6)', fontSize:'.8rem', lineHeight:1.55, margin:0 }}>Ton temps sur 200m à effort maximal soutenable</p>
                  </div>
                </div>
                <p style={{ color:'rgba(255,255,255,.3)', fontSize:'.72rem', margin:0, textAlign:'center', fontStyle:'italic' }}>Wakayoshi K. et al., Journal of Applied Physiology, 1992</p>
              </div>
            </div>
            <div style={{ flex:'1 1 320px' }}>
        ...
        <Inner max={1100}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(min(260px, 100%), 1fr))', gap:'1.25rem' }}>
            {[
              ...
              <div key={s.t} style={{ background:'rgba(255,255,255,.04)', borderRadius:16, padding:'1.5rem', border:'1px solid rgba(139,47,201,.1)' }}>
                ...
                <p style={{ color:'rgba(255,255,255,.6)', fontSize:'.875rem', lineHeight:1.7, textAlign:'justify', margin:0 }}>{s.d}</p>
              </div>
            ))}
          </div>
        </Inner>
